...
-- Grant usage on database and schema to SYSADMIN role
GRANT USAGE ON DATABASE ATS_DB TO ROLE SYSADMIN;
GRANT USAGE ON SCHEMA ATS_DB.ATS_SCHEMA TO ROLE SYSADMIN;

-- Grant SELECT for dashboard and query access
GRANT SELECT ON ALL TABLES IN SCHEMA ATS_DB.ATS_SCHEMA TO ROLE SYSADMIN;
GRANT SELECT ON ALL VIEWS IN SCHEMA ATS_DB.ATS_SCHEMA TO ROLE SYSADMIN;
GRANT SELECT ON ALL DYNAMIC TABLES IN SCHEMA ATS_DB.ATS_SCHEMA TO ROLE SYSADMIN;

-- Grant future SELECT privileges for new objects
GRANT SELECT ON FUTURE TABLES IN SCHEMA ATS_DB.ATS_SCHEMA TO ROLE SYSADMIN;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA ATS_DB.ATS_SCHEMA TO ROLE SYSADMIN;
GRANT SELECT ON FUTURE DYNAMIC TABLES IN SCHEMA ATS_DB.ATS_SCHEMA TO ROLE SYSADMIN;

-- ⚠️ CRITICAL: Grant INSERT permission for Kafka Connector
-- The Snowflake Kafka Connector requires INSERT permission to write data
-- This was discovered during testing - connector fails without this permission
GRANT INSERT ON TABLE ATS_DB.ATS_SCHEMA.ATS_RAW_JSON TO ROLE SYSADMIN;

-- Optional: Grant to PUBLIC role for read-only dashboard access
